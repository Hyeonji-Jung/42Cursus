...
class Page:
    ...
    def __norm_html(self, elem: Elem()) -> bool:
        ...
        if isinstance(elem, Html) and len(elem.content) == 2 and type(elem.content[0]) == Head and type(elem.content[1]) == Body:
            if (all(self.__norm_html(el) for el in elem.content)):
                return True
        if isinstance(elem, Head) and [isinstance(el, Title) for el in elem.content].count(True) == 1:
            if (all(self.__norm_html(el) for el in elem.content)):
                return True
        if isinstance(elem, (Body, Div)) and all([isinstance(el, (H1, H2, Div, Table, Ul, Ol, Span)) or type(el) == Text for el in elem.content]):
            if all(self.__norm_html(el) for el in elem.content):
                return True
        if isinstance(elem, (Title, H1, H2, Li, Th, Td)) and len(elem.content) == 1 and type(elem.content[0]) == Text:
            return True
        if isinstance(elem, P) and all([isinstance(el, Text) for el in elem.content]):
            return True
        if isinstance(elem, Span) and all([isinstance(el, (Text, P)) for el in elem.content]):
            if all(self.__norm_html(el) for el in elem.content):
                return True
        if isinstance(elem, (Ul, Ol)) and len(elem.content) > 0 and all([isinstance(el, Li) for el in elem.content]):
            if all(self.__norm_html(el) for el in elem.content):
                return True
        if isinstance(elem, Tr) and len(elem.content) > 0 and all([isinstance(el, (Th, Td)) for el in elem.content]) and all([type(el) == type(elem.content[0]) for el in elem.content]):
            return True
        if isinstance(elem, Table) and all([isinstance(el, Tr) for el in elem.content]):
            return True
        return False
...
#    print(f'html_page: {Page(html).is_valid()}\n')